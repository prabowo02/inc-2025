// TLE solution
// Uses segtree of XOR bases instead of sparse table
// Additional log factor in complexity
...
#define rc (lc | 1)
    ...
    SegtreeBasis(int _n, const vector<ll> &base) : n(_n), seg(4*_n + 1) {
        build(1, 1, n, base);
    }

    void build(int pos, int l, int r, const vector<ll> &base) {
        if (l == r) {
            seg[pos].insert(base[l]);
            return;
        }
    ...
    XORBasis query(int pos, int l, int r, int ql, int qr) {
        if (l > r || ql > r || l > qr || ql > qr) return XORBasis();
        if (ql <= l && r <= qr) return seg[pos];

        return merge(query(lc, l, m, ql, qr), query(rc, m+1, r, ql, qr));
    }
    XORBasis query(int ql, int qr) { return query(1, 1, n, ql, qr); }
...
int main() {
    ...
    for (int i = 1; i <= n; i++) {
        ...
        while (l <= r) {
            int m = (l + r) / 2;
            XORBasis basis = segbasis.query(i, m);
